...
def get_local_time():
    return time.time()  # Use actual current time
...
def main():
    ...
    while not time_adjusted:
        ...
        if server_res["operation"] == "time_req":
            ...
            print(f"Sending local time to server: {datetime.fromtimestamp(client_local_time)}")
        ...
        if server_res["operation"] == "time_adj":
            ...
            print(f"Adjusted time: {datetime.fromtimestamp(client_local_time)}")
